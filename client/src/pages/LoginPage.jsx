...
export default function LoginPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // ✅ Feature 1: After login, navigate straight to Dashboard
  ...
  return (
    <div className="relative min-h-screen overflow-hidden bg-gradient-to-br from-slate-50 via-white to-indigo-50">
      ...
      <motion.div
        aria-hidden
        className="pointer-events-none absolute -bottom-16 -left-16 h-80 w-80 rounded-full bg-blue-300/30 blur-3xl"
        initial={{ opacity: 0, scale: 0.8 }}
        animate={{ opacity: 1, scale: 1 }}
        transition={{ duration: 1.4, delay: 0.2, ease: "easeOut" }}
      />

      <main className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 pt-28 pb-20 grid grid-cols-1 lg:grid-cols-2 gap-10 items-center">
        ...
        <section>
          ...
          >
            {/* ✅ Feature 4: Introduction explaining what users can do */}
          ...
          <div id="how-it-works" className="mt-8">
            <div className="inline-flex items-center gap-2 rounded-xl border border-slate-200 bg-white px-4 py-2 text-sm text-slate-700 shadow-sm">
              ...
              {/* ✅ Feature 2: Sign up link to redirect users */}
        ...
        <section className="relative">
          <AnimatePresence mode="wait">
            ...
            >
              ...
              <form onSubmit={handleSubmit} className="bg-white/90 backdrop-blur rounded-3xl shadow-xl ring-1 ring-slate-200 p-6 sm:p-8">
                <div className="mb-6 text-center">
                  <h2 className="text-2xl font-bold tracking-tight text-slate-900">Welcome back</h2>
                  <p className="mt-1 text-sm text-slate-600">Login to access your dashboard and keep momentum.</p>
                </div>
                ...
                <input
                  type="email"
                  className="mt-1 mb-4 w-full rounded-xl border border-slate-300 px-3 py-2 outline-none focus:ring-2 focus:ring-indigo-400"
                ...
                <input
                  type="password"
                  className="mt-1 mb-2 w-full rounded-xl border border-slate-300 px-3 py-2 outline-none focus:ring-2 focus:ring-indigo-400"
                  placeholder="••••••••"
                  value={password}
                ...
                <motion.button
                  type="submit"
                  disabled={loading}
                  className="group relative w-full inline-flex items-center justify-center gap-2 rounded-xl bg-indigo-600 px-4 py-2.5 text-white font-semibold shadow hover:bg-indigo-700 focus:outline-none focus-visible:ring-2 focus-visible:ring-indigo-400 disabled:opacity-70"
                  whileTap={{ scale: 0.98 }}
                >
                ...
                <p className="mt-4 text-center text-sm text-slate-600">
                  ...
                  {/* ✅ Feature 2 again: clear CTA to signup */}
      ...
      <footer id="faq" className="border-t border-slate-200 bg-white/60 backdrop-blur">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-8 grid sm:grid-cols-3 gap-6 text-sm text-slate-600">
          <div>
            <div className="font-semibold text-slate-800">Why use Job Tracker?</div>
            <p className="mt-2">Centralize applications, never miss deadlines, and stay interview‑ready with notes and reminders.</p>
          </div>
          <div>
            <div className="font-semibold text-slate-800">Security</div>
            <p className="mt-2">Your data is encrypted in transit. You control what you store.</p>
          </div>
          <div>
            <div className="font-semibold text-slate-800">Support</div>
            <p className="mt-2">Questions? Reach us via the in‑app chat after you sign in.</p>
          </div>
        </div>
      </footer>
    </div>
